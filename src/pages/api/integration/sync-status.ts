...
export default async function handler(
  ...
  try {
    ...
    // Ashborn uses /integrations/:id/sync/status for connection sync status
    const params: ApiPropsType = {
      url: `/integrations/${integration_id}/sync/status`,
      method: "get",
      headers: {
    ...
    return res.status(status).json(progressData);
  } catch (err) {
